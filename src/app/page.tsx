...
export default function HomePage() {
  ...
  const handleFileChange = async (
    ...
    try {
      ...
      toast.error("Error processing files", {
        description:
          err.message || "Could not load photos from the selected folder.",
      });
      setAllPhotos([]);
      setVisiblePhotos([]);
      setSelectedFolderName(null);
    ...
    (
      ...
      for (const meta of nextBatchMetadata) {
        try {
          ...
          console.error(
            `Failed to create Object URL for ${meta.name}:`,
            urlError
          );
  ...
  ); // Dependencies for when called by InfiniteScroll
    ...
    <>
      ...
      <div className="container mx-auto p-4 md:p-8 space-y-6">
        <div className="flex items-end justify-between">
          ...
          <div className="ml-2">
            <h5 className="text-xs">Select Photos From Your Computer</h5>
        ...
        {selectedPhotoList.length > 0 ? (
          <Card className="h-[200px] sticky top-0 z-20 bg-white">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle>
                Selected Photos ({selectedPhotoList.length})
              </CardTitle>
              <Button
                variant="outline"
                size="sm"
                onClick={handleCopyToClipboard}
                disabled={isCopying || selectedPhotoList.length === 0}
            ...
            <CardContent>
              <ScrollArea className="h-20 w-full rounded-md border p-4">
                <ul className="list-decimal list-inside space-y-1 text-sm">
                  ...
                  {selectedPhotoList.map((photoName) => (
        ...
        {selectedFolderName && !isLoading && allPhotos.length > 0 && (
          <Card className="m-2">
            <CardHeader>
              <CardTitle>
                Photos in &quot;{selectedFolderName}&quot; ({allPhotos.length}{" "}
                found)
              </CardTitle>
            </CardHeader>
            <CardContent>
      ...
      {viewerIndex >= 0 &&
        lightboxSlides.length > 0 && ( // Conditionally render lightbox only when needed
          <Lightbox
            open={viewerIndex >= 0}
            ...
            plugins={[Zoom]}
            zoom={{
              /* ... zoom settings ... */ maxZoomPixelRatio: 3,
      ...
      <Card
        ...
        onKeyDown={(e) => {
          if (e.key === "Enter" || e.key === " ") {
            e.preventDefault();
            onSelectToggle(photo.name);
          }
        }} // Keep Enter/Space for selection
      >
